...
[`standard config`](https://www.npmjs.com/package/stylelint-config-standard)는 [`built-in rules`](https://stylelint.io/user-guide/rules)의 절반 정도만 설정이 되어있다.
...
그 중 가장 대표적인 것이 [`order plugin`](https://www.npmjs.com/package/stylelint-order)를 포함하고 있는 [`recess order config`](https://www.npmjs.com/package/stylelint-config-recess-order)이다.
